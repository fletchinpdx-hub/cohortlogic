...
(function () {
  ...
  function injectStyles() {
    ...
    var css =
      ...
      '#cb-tour-dim{position:fixed;inset:0;background:rgba(30,58,95,.55);opacity:0;transition:opacity .2s ease}' +
      '#cb-tour-dim.show{opacity:1}' +
      // Spotlight: box-shadow creates the dim + a cutout hole around the anchor
      '#cb-tour-spot{position:fixed;border-radius:10px;box-shadow:0 0 0 9999px rgba(30,58,95,.62);' +
      ...
      '#cb-tour-card h3{margin:0 0 8px;font-size:18px;font-weight:800;color:var(--navy, #1E3A5F);line-height:1.25}' +
      ...
      '#cb-tour-dots span.on{background:var(--teal, #2A9D8F)}' +
      ...
      '.cb-tour-btn-primary{background:var(--teal, #2A9D8F);color:#fff}' +
      '.cb-tour-btn-primary:hover{background:var(--teal-dark, #21867a)}' +
