...
**Overly-permissive policy check (semantic, not mechanical):** `rls_enabled`/`policy_count` alone can't tell a properly-scoped policy from a rubber-stamp one — `policy_count > 0` looks fine even when that one policy is `using (true)` for `to authenticated` or `to anon` on a table holding PII. (This is exactly how `contact_submissions`/`newsletter_subscribers` slipped through on 2026-07-16 — caught only because the agent happened to read `marketing_tables.sql` directly.) So for every table `security_rls_snapshot()` returns, also read its migration source under `supabase/migrations/` and check each policy definition: if a SELECT/UPDATE/DELETE policy is scoped `to authenticated` or `to anon` with `using (true)` (or no meaningful `using` condition) on a table that isn't intentionally public, emit `severity: "high"`, `check_id: "rls.<table>.overpermissive-policy"`, quoting the policy name and table in the detail. Tables that ARE intentionally wide-open for anon (e.g. `contact_submissions`/`newsletter_subscribers`' own INSERT policies, `sessions`/`events` anon+authenticated INSERT) are fine — this check is about SELECT/UPDATE/DELETE exposing existing data, not intentional public-INSERT collection points. When nothing overpermissive is found for a table, still emit `pass: true` for that table's own check_id so a later fix can auto-resolve.
