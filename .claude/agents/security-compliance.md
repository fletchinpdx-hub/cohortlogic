...
**Overly-permissive policy check (semantic — reads live policy bodies):** `rls_enabled`/`policy_count` alone can't tell a properly-scoped policy from a rubber-stamp one — `policy_count > 0` looks fine even when that one policy is `using (true)` for `to authenticated` or `to anon` on a table holding PII. (This is exactly how `contact_submissions`/`newsletter_subscribers` slipped through on 2026-07-16 — caught only because the agent happened to read `marketing_tables.sql` directly.) As of the v2 `security_rls_snapshot()`, each returned row carries a `policies` jsonb array — one object per policy: `{name, cmd, permissive, roles, qual, with_check}`. **Use `policies` as the source of truth, NOT migration files** — it sees every policy including those authored directly in the SQL Editor (`profiles`/`features`/`feedback`/`sessions`/`events`), which the old migration-grep was blind to. For every table, inspect each policy whose `cmd` is `SELECT`, `UPDATE`, `DELETE`, or `ALL` **and** whose `roles` include `anon`, `authenticated`, or `public` (skip pure-`INSERT` policies — those are intentional public collection points). Take the governing expression (`qual` for SELECT/DELETE/ALL; also `with_check` for UPDATE):
- **Hard-flag `high`** (`check_id: "rls.<table>.overpermissive-policy"`) when that expression is SQL-null or normalizes to `true` (strip parens/whitespace, lowercase) — unconditional read/write of existing rows, the exact `contact_submissions` bug. Quote the policy name + table.
- **Soft-flag `medium`** (same check_id) when the expression gates only on "is a user logged in" — references `auth.role()`/`auth.uid()` (or `… is not null`) with NO row-scoping predicate (no `my_school_id()`, `can_access_product(...)`, `is_admin()`/`is_super_admin()`, and no ownership comparison like `user_id = auth.uid()`). That's "any authenticated user sees everything."
- **Pass** (`pass: true`) when every such policy carries a real scoping predicate (one of those functions, or a row-ownership / `school_id` comparison).

**Intentionally-broad allowlist — do NOT flag** (non-PII, public/aggregate by design; emit `pass: true` so the check stays resolvable): `features` (public feature flags, read-anywhere) and the authenticated-read analytics on `sessions` and `events` (anonymous Class Builder usage, no student data). Public-INSERT collection points (`contact_submissions`/`newsletter_subscribers`/`feedback`/`error_logs` INSERT) are likewise fine — this check is about SELECT/UPDATE/DELETE exposing existing data. Always emit each table's `rls.<table>.overpermissive-policy` result (pass or fail) so a later fix can auto-resolve it.
