...
- **Trial / entitlement gating (freemium, Phase 1).** `js/entitlements.js` (`Entitlements`) reads the user's access at boot via the `my_entitlement()` RPC — **server-authoritative** (can't be spoofed from localStorage), **fails safe** to the limited tier on error, and **super admins are never gated**. Access levels: `full` (paid — no gating), `trial` (limited), `expired` (hard lockout wall). Enforcement is **client-side only** (Class Builder is fully client-side; the roster never goes server-side, so we deliberately do NOT move the algorithm/export to a server to harden the gate — see the caveat below). Trial limits: **only the "1st grade"** is unlocked (`Entitlements.unlockedGrade()` normalizes "1"/"1st"/"First Grade"…); **no export, no Save Session** (print isn't an in-app feature here). The single data chokepoint is `runBalancingAlgorithm` → `activeGrades()` (typeof-guarded so the unit test still runs standalone): a trial only ever *computes* the unlocked grade, so no other grade's arrangement exists to leak. UI gates (all read `cbFull()`/`activeGrades()` from app.js): `classes.js` greys non-trial grade cards + the split card; `students.js` locks the grade filter and scopes the roster/rule dropdowns; `results.js` blocks export with an upgrade modal; `session.js` blocks Save. Every gate fires a `cb_gate_hit` event. **Set plans from the admin Billing tab** (tier `individual`/`school`/`district`, status, seats, and a "Start 60-day trial" button) — these write `subscriptions.tier` / `.status` / `.trial_ends_at`, which `my_entitlement()` resolves per the user's school (a district-provisioned school has its own row). Schema: `supabase/migrations/entitlements.sql`. **Caveat (accepted):** the entitlement *value* is server-authoritative but the *enforcement* is soft (bypassable via dev tools) — the right trade for a client-only app whose privacy promise is "the roster never leaves the browser." Tiers: Individual (1 seat), School (default 5), District (provisions member schools, each a 5-seat row). Trial is time-limited (default 60 days) then hard-locks.
...
### 3. Schedule Builder (`schedule-app.html`)
Building schedule builder for school administrators. All four phases (Setup → Build → Detail → Export) are built and live.
...
- Phase 1 — Build: Building Schedule (locked until grades configured)
...
- `iaSchedule[day][iaId][slot]` = `{ allocId, targetType, targetId, note }` — IA assignments. `targetType` is `'grade'` (coverage; `targetId` = grade), `'block'` (**school-wide** duty coverage; `targetId` = block id — see scope below), `'own_lunch'` (engine-reserved IA lunch; `allocId` optional), or `'break'` (engine-reserved break; no alloc). **Written by the placement engine `placeIAs()`, not by hand-assignment** — see the IA scheduling section below. `_entryBtId(day, entry, slot)` resolves an entry's underlying block (grade → building schedule; block → `targetId`) for cell color + run-grouping.
...
- `iaCoverage[]` = `[{ id, blockId, subId|null, scope, grades[], iasPerGrade, allowedAllocIds[] }]` — the **coverage plan** (IA Assignment tab). `scope` is `'grade'` (per-grade: `iasPerGrade` aides for EACH grade in `grades[]`) or `'school'` (**school-wide**: `iasPerGrade` is the TOTAL aide count for one shared duty, `grades[]` ignored → ONE requirement, `targetType:'block'`). `_isSchoolWideEligible(blockId)` = a uniform block type with a single school-wide time (arrival/dismissal/MM/custom) — those get a per-row **Per grade / School-wide** toggle (default school-wide); lunch/recess/instructional are per-grade only. `_normalizeCoverageScope()` backfills `scope` on load (eligible → school, so an old dismissal row stops multiplying). School-wide occurrences come from `_iaSchoolBlockOccurrences` (found once across grades). The list also contains two special **personal-time** rows `{ id, kind:'ia_lunch'|'ia_breaks' }` (no block/grade/count) whose position sets when each aide's lunch/breaks are reserved relative to coverage. Row **order = fill priority** (top staffed first; ↑/↓ reorder). Drives `placeIAs()`. Persisted in the `.cohortlogic` file parallel to `iaAllocations`.
- **Recess variants:** the coverage plan splits recess into two rows via a virtual `subId` — `IA_RECESS_LUNCH` (`'__recess_lunch'`) and `IA_RECESS_FREE` (`'__recess_free'`), both `blockId: 'bt_recess'` (schedule-ia.js, engine region). The building schedule still stores plain `bt_recess`; `_iaBlockOccurrences` forms runs on the base block then filters by `_recessBlockInfo(...).lunchAdjacent` (unknown → treated as standalone so it's still covered). This lets the lunch-connected recess be staffed differently (e.g. 2 IAs vs 1). `_migrateRecessCoverage()` (idempotent, runs in `renderIAAssignmentView`) splits legacy `bt_recess`/null rows into both variants. `_iaBlockLabel()` renders the friendly names; a hint warns if only one variant is covered. Test: `tests/ia-recess-variant.test.js`.
...
- `school.iaDefaults` = `{ startTime, endTime, lunchWindowStart, lunchWindowEnd, lunchDuration, breakCount, breakDuration }` — set on the **School Info** tab ("IA Defaults" section). Pre-fill values for a **NEW** IA on the Staff Roster (applied when the role dropdown is switched to IA, `!existing`). Editing them only affects IAs added afterward — existing IAs keep their own values.
...
- `js/schedule-grid.js` (~3,450 lines, was ~6,177 pre-split) — CORE: Building Schedule grid render/cells, drag-to-move, resize, context menu, undo, the consolidated warnings panel, conflict rendering/banners, the placement algorithm (specials scheduling incl. `buildSpecialsSchedule`/`findGradeFixedTime`, grade pairings/`placePairedBlocks`, `_populateGradeData`, `rebuildPlacement()`), and `printScheduleGrid` (a shared print utility used by IA/specials/class views — intentionally NOT extracted, stays here)
- `js/schedule-ia.js` (extraction 1) — everything IA: the **IA Assignment tab** (`renderIAAssignmentView` — budget categories + coverage plan + Place IAs), the **placement engine** (`placeIAs`), the **IA Schedule view** (All IAs + Individual IA), the assignment editors (`openIAAssignmentEditor` edit/reassign/partial/delete — preserves the entry's `targetType`; `openIAAddAssignment` add on open cells — collapses school-wide blocks to one target and has an **"Also assign to"** multi-aide picker so one Add staffs several aides at once, skipping any that are busy), the placement-issues panel (`_iaPlacementIssuesHtml`), and duty blocks/panel. State: `iaSchedUI`, `iaDrag`. (The old master-grid "Assign IAs" mode + `iaMasterState` were removed in the IA rework.)
...
- `getSpecialsCoverageReport()` — detects classes with missing specials (day-level gaps not caught by grade-level failure); called from building schedule and Specials Schedule view
...
- **IA scheduling model (rework — see `docs/ia-rework-plan.md`).** IAs are placed **automatically**, not hand-assigned. Config lives on the **IA Assignment tab** (budget categories + coverage plan, both auto-save); **Place IAs** runs `placeIAs()` (wipes `iaSchedule` — but not duties — behind a confirm, then rebuilds). Results show on the **IA Schedule tab**, where you adjust them.
  - **`placeIAs()`** — pure, deterministic. Organized around **weekly recurring requirements** (one per coverage-row × grade with ≥1 occurrence). **Every** requirement — instructional AND duty (`IA_DUTY_BLOCKS` = lunch + recess + arrival `bt_arr` + dismissal `bt_dis`) — reuses ONE consistent team all week in `placeReq`: the same aide keeps the same block/grade every day (an aide's slot never shows lunch some days, recess others), and a substitute on a day the team can't cover is flagged as an inconsistency. Spread across aides comes from the TEAM SELECTION, not day-to-day rotation. **Team selection weighs actual availability FIRST** (`daysFree` — in-hours AND `free()` on that day, i.e. not already booked by an earlier/higher-priority row): aides who can cover EVERY day of the block are preferred, then duty balance/preference decides among them. Ranking on hours alone (the old `daysCoverable`) could pick an aide already booked on one day, forcing a substitute and reporting a bogus "different IAs on different days" while an aide free all week sat unused — see `tests/ia-team-availability.test.js`. Duty ranking (`rankKey`, best-first): **(1) least accumulated duty of THIS type** (`dutyMin`), **(2) grade preference** (a factor, not a monopoly), **(3) overall load**. Because `dutyMin` accumulates a FULL week per duty assignment, once an aide takes one duty their next-duty rank drops and the next duty goes to a different, less-loaded aide — so no aide (not even one who prefers every grade) soaks up all the recess duty. Instructional keeps grade-preference-first. **Priority order = the coverage-plan list order, and it now includes personal time.** The list carries two special rows, `{kind:'ia_lunch'}` and `{kind:'ia_breaks'}` (seeded at the top by `_ensureIAPersonalRows`, reorderable via the ↑↓ arrows like any row). `placeIAs` walks the list top-to-bottom: at an `ia_lunch`/`ia_breaks` row it runs `reserveOwnLunches`/`reserveBreaks` (demand-aware, dodging busy times); at a coverage row it places that row's `reqs` (grouped by list index `rowIndex`, hardest-to-staff grade first within the row). So dragging "IA's lunch" above the coverage rows reserves lunch first (aides always get it); below, coverage wins contested slots and lunch works around what's left. Files predating the rows fall back to reserving personal time first. **No block type is special-cased** (the old "kids' lunches first" hardcode is gone — put that row at the top of the list to prioritize it). **Hard** constraints: within the IA's hours, no double-booking. **Soft**: for instructional, grade preference leads then load; for duty, the `rankKey` above (weekly duty parity → preference → load). Budget is a **soft** per-day cap (`hoursPerDay`) — warns, never blocks. Preference/parity/budget only decide *who* fills a slot, never *whether* — a gap is recorded only when no eligible IA is free.
  - **Own lunch / breaks: same clock time every day, spread across IAs.** `reservePersonalTime` scores each candidate run by `busy×1000 + demand` (busy = IAs already on personal time at that slot → forces a spread across IAs; demand = per-slot coverage need → biases toward low-demand times). It then picks ONE start time — the one free on the MOST days, tie-broken by cost — and reserves it at that same time on every day it fits; a day it genuinely can't fit is reported `ownLunchUnplaced`/`breaksUnplaced` rather than scattered to a different time (an aide's lunch is never at 11:00 one day and 11:30 the next). This fixed a real bug where first-fit clustered every IA's lunch at the window start and stripped coverage. Breaks: default 1 × 15 min (per-IA `breaks`), **never in the first or last hour** of the shift; multiple breaks split the mid-day into segments.
  - **IA time range** — the IA Schedule grid AND the placement engine scan `_iaAutoFillSlots(day)`, NOT `_autoFillSlots`. It's the instructional day (`_autoFillSlots`) widened to the earliest IA `startTime` / `studentCampusStart` and latest IA `endTime` / `studentCampusEnd`, so pre-bell coverage and arrival/dismissal duty (7:25 duty for a 7:25-start aide) are visible and placeable. Instructional/master views keep `_autoFillSlots` (first bell → dismissal).
  ...
  - **IA Schedule view:** "All IAs" (`buildIAGrid`, one day) + Individual (`buildIndividualIAGrid`, week). own_lunch/break render as "Own lunch"/"Break" (via `_iaTargetLabel`). Click a **filled** cell → `openIAAssignmentEditor`: a grade/block assignment can reassign IA, shorten to a partial block, change category/note, or delete (no-double-booking guard); a **personal** entry (own lunch / break) instead gets a **"Move to"** picker over the aide's hours — moving it onto an occupied slot is allowed but `confirm()`-warned (it replaces what's there), so aides aren't locked into the engine's placement. **Personal time can be charged to a budget category** (own lunch AND breaks) right here rather than in staff config — `getIASummaryForIA` already sums any entry carrying an `allocId` regardless of `targetType`, so it flows into the budget bar/summary/CSV with no accounting change. Click an **open** cell → `openIAAddAssignment` (pick a grade/school-wide target running at that time, the range, the category, an **"Also assign to"** multi-aide picker, and an **"Also apply to"** multi-DAY picker — a day is skipped unless the same block runs across those exact slots, an aide is skipped where already busy, and everything skipped is reported). `_iaAssignmentRun` groups a contiguous same-alloc/target run. Editors live on `document.body` (id `ia-assign-editor`); every committed action **closes the popover** and re-renders via `_rerenderIAKeepingScroll()` (captures/restores the `.ia-ind-grid-wrap`/`.ia-all-grid-wrap` scrollTop so the page doesn't jump to the top).
  - **Engine tests:** `tests/ia-engine.test.js` (core placement), `tests/ia-breaks.test.js` (personal time + breaks), `tests/ia-spread.test.js` (lunch/break spread + coverage), `tests/ia-priority.test.js` (coverage-list row order drives fill priority), `tests/ia-recess-variant.test.js` (lunch vs standalone recess routing), `tests/ia-duty-consistency.test.js` (duty keeps the same aide all week while different duties spread across aides), `tests/ia-schoolwide.test.js` (school-wide scope staffs a fixed count, not per-grade), `tests/ia-personal-order.test.js` (IA lunch/breaks reserved at their list position), `tests/ia-personal-sametime.test.js` (lunch/breaks land at the same clock time every day), `tests/ia-team-availability.test.js` (team selection prefers an aide free EVERY day over one already booked), `tests/ia-block-move-follow.test.js` (coverage travels with a moved block). Not in the pre-deploy gate; run with `node tests/ia-*.test.js` — **all must pass; a failing one means real drift** (`ia-recess-rotate.test.js` was deleted when duty rotation was replaced by week-long consistency; don't resurrect it).
- **Assign IAs from the Building Schedule** (`openMasterIAAssign`, schedule-ia.js) — two entry points: the **"Assign IAs" palette tool** (`gridUI.tool === 'assign-ia'`; intercepted FIRST in `onPointerDown` so it can't fall through to the resize handle, and carries no `activeBtId`) and **"Assign IAs…"** on the block right-click menu. Lists every aide with an availability state — Free / Already assigned / Busy · *what* / Outside hours — **disabled, not hidden**, so the reason stays visible. Carries a budget category + an "Also apply to" day picker (only days the same block runs at that time). School-wide-eligible blocks assign as `targetType:'block'`, everything else as the grade. Rebuilds the grid so the IA dots refresh. Supplements automatic placement — Place IAs still wipes and rebuilds `iaSchedule`, including these manual assignments.
- **IA coverage follows a moved block** — `_shiftIACoverageForMove` (schedule-grid.js), called from `commitMove`, translates each covering entry by the same offset within the block so a slid block keeps its aides at BOTH ends. Without it `_cleanupStaleIAAssignments` only trimmed the vacated head slot, leaving a hole at the block's new tail. Lifts all entries before re-placing (so a 5-min slide can't collide with itself), won't clobber an unrelated assignment at a destination slot, drops overflow past end-of-day, and is **skipped for conflict-sourced moves** (the vacated slot keeps its primary block, whose coverage must stay put). Test: `tests/ia-block-move-follow.test.js`. NOTE: **resize** does not yet extend coverage into newly added minutes (shrink is handled by the stale cleanup).
...
- `getIAsForBlock(day, grade, slot)` — returns IA assignments for a building schedule block; dots render bottom-right of each filled cell (`.ia-block-ind`, `position: absolute; bottom: 4px; right: 5px`). Dot color = allocation category color, NOT staff member's `.color`
...
3. Building Schedule: `buildSpecialsSchedule()` auto-places `bt_spec|sp_id` blocks; per-special colors used throughout
...
### Signup gating — school email only (trial Phase 2)
`js/signup.js` holds `FREE_EMAIL_DOMAINS` + `isAllowedEmailDomain()`; the submit handler rejects personal providers (gmail/yahoo/outlook/icloud/proton/…) **before** calling `auth.signUp`, and `signup.html` labels the field **"School email"** with a hint so the rule is stated up front. This is a **front-door deterrent** — it runs in the browser, so a determined person could still create a *pending* account. The real gate is unchanged: **every signup requires manual approval** (Approvals queue), and the post-signup message already sets that expectation. A server-side backstop in `handle_new_user()` is the available hardening if queue junk becomes a problem (not done — that trigger is the critical auth path and shouldn't be edited blind).
...
5. `supabase/migrations/subscriptions.sql` — per-school subscription/billing ledger (super-admin-only RLS; money in integer cents)
6. `supabase/migrations/stripe_billing.sql` — Stripe linkage columns on `subscriptions` (webhook is the only writer)
7. `supabase/migrations/entitlements.sql` — `subscriptions.tier` + `.trial_ends_at`, and the `my_entitlement()` SECURITY DEFINER RPC that is the ONLY thing the client reads for access gating (returns access/tier/status/trial_ends_at for the caller's own school — never fees or Stripe ids; super admins always resolve to `full`)
...
public/
  ...
  schedule-app.html     — Schedule Builder app
...
- `qa-schedulebuilder` — Schedule Builder (`schedule-app.html`): login/gate → seed a minimal school → render + exercise Building Schedule (core) and each extracted feature view (IA, Specials view, Class view, Export); also confirms the shared feedback widget and the Synchronized-Blocks non-overlap rule.
- `qa-signup` — the public signup page (`signup.html`): the school-email gate. Drives the **reject** path end-to-end (safe — the domain check runs before `auth.signUp`, so no account is created) and verifies the **accept** path *without submitting*, since a real signup creates an `auth.users` row + pending profile someone must clean up. Needs no credentials.
...
### Go-to-market (trial → paid)
- **Phase 3 — Stripe (NOT started).** Phases 1 (Class Builder gating + entitlements + admin Billing controls) and 2 (school-email signup gating) are live. Remaining: self-serve checkout + trial→paid conversion. The columns already exist (`stripe_billing.sql`) and `my_entitlement()` already maps Stripe's lifecycle (`trialing→trial, active→active, past_due→past_due, canceled→cancelled`), so the work is the Checkout flow + a webhook Edge Function (service-role; the ONLY writer of the Stripe columns) + surfacing plan state to the user.
- **Schedule Builder trial gates** — not built. Will reuse the same `entitlements.js` layer; needs its own per-view gate spec (the Class Builder equivalent of "1st grade only, no export/print/save").
- **Seat enforcement** — `license_count` is recorded but not enforced (School = 5 by default). Today the manual-approval queue is the de facto limiter.

### Schedule Builder
- **IA coverage on block RESIZE** — a moved block now carries its coverage (`_shiftIACoverageForMove`), but *growing* a block via the resize handle still leaves a gap at the new end (shrink is handled by `_cleanupStaleIAAssignments`). Intended rule if built: if coverage exactly spanned the block before the resize, grow it to match; otherwise leave the partial alone.
...
- **Dismissal Duty (`bt_dis`)** — still appears in the Building Schedule palette (Arrival Duty was removed in v142); remove via `PALETTE_EXCLUDE` if it's non-paintable in the user's workflow.
