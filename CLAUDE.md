...
- `iaCoverage[]` = `[{ id, blockId, subId|null, grades[], iasPerGrade, allowedAllocIds[] }]` — the **coverage plan** (IA Assignment tab). Each row = "cover this block, for these grades, N IAs per grade, funded by these categories." Row **order = fill priority** (top staffed first; ↑/↓ reorder). Drives `placeIAs()`. Persisted in the `.cohortlogic` file parallel to `iaAllocations`.
