...
- **IA scheduling model (rework — see `docs/ia-rework-plan.md`).** IAs are placed **automatically**, not hand-assigned. Config lives on the **IA Assignment tab** (budget categories + coverage plan, both auto-save); **Place IAs** runs `placeIAs()` (wipes `iaSchedule` — but not duties — behind a confirm, then rebuilds). Results show on the **IA Schedule tab**, where you adjust them.
  - **`placeIAs()`** — pure, deterministic. Organized around **weekly recurring requirements** (one per coverage-row × grade with ≥1 occurrence). **Instructional** blocks reuse ONE consistent IA across all days (same aide keeps the class; per-day fallback flagged as an inconsistency). **Duty** blocks (`IA_DUTY_BLOCKS` = lunch + recess + arrival `bt_arr` + dismissal `bt_dis`) instead **ROTATE across aides** via `req.isDuty` in `placeReq` (team = []; per-day fill), so supervision is shared rather than one person every day; rotation is NOT flagged as an inconsistency. Duty ranking (`rankKey`, best-first): **(1) least duty already THAT DAY** (`dutyDayMin` — so an aide already on a recess yields to a free one, spreading duty *within* a day), **(2) least duty across the week** (`dutyMin`), **(3) grade preference** (still a factor — breaks ties), **(4) overall load**. This ordering is what stops an aide who prefers *every* grade from soaking up all the recess duty; instructional blocks keep grade-preference-first. **Priority order = the coverage-plan list order, and it now includes personal time.** The list carries two special rows, `{kind:'ia_lunch'}` and `{kind:'ia_breaks'}` (seeded at the top by `_ensureIAPersonalRows`, reorderable via the ↑↓ arrows like any row). `placeIAs` walks the list top-to-bottom: at an `ia_lunch`/`ia_breaks` row it runs `reserveOwnLunches`/`reserveBreaks` (demand-aware, dodging busy times); at a coverage row it places that row's `reqs` (grouped by list index `rowIndex`, hardest-to-staff grade first within the row). So dragging "IA's lunch" above the coverage rows reserves lunch first (aides always get it); below, coverage wins contested slots and lunch works around what's left. Files predating the rows fall back to reserving personal time first. **No block type is special-cased** (the old "kids' lunches first" hardcode is gone — put that row at the top of the list to prioritize it). **Hard** constraints: within the IA's hours, no double-booking. **Soft**: for instructional, grade preference leads then load; for duty, the `rankKey` above (within-day spread → weekly duty parity → preference → load). Budget is a **soft** per-day cap (`hoursPerDay`) — warns, never blocks. Preference/parity/budget only decide *who* fills a slot, never *whether* — a gap is recorded only when no eligible IA is free.
  ...
  - **Engine tests:** `tests/ia-engine.test.js` (core placement), `tests/ia-breaks.test.js` (personal time + breaks), `tests/ia-spread.test.js` (lunch/break spread + coverage), `tests/ia-priority.test.js` (coverage-list row order drives fill priority), `tests/ia-recess-variant.test.js` (lunch vs standalone recess routing), `tests/ia-recess-rotate.test.js` (duty blocks rotate day-to-day; instructional stays consistent), `tests/ia-schoolwide.test.js` (school-wide scope staffs a fixed count, not per-grade), `tests/ia-personal-order.test.js` (IA lunch/breaks reserved at their list position). Not in the pre-deploy gate; run with `node tests/ia-*.test.js`.
