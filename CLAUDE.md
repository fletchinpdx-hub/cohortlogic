...
- `buildSpecialsSchedule(force=false)` — computes class-level rotation, finds grade-wide time slots, writes bt_spec|spId to masterSchedule and specialsSchedule. Unforced calls SKIP the rebuild while every class already has a specialsSchedule entry (v130) — a full rebuild wipes manual specials moves on the master grid. Rebuild triggers: specialsSchedule empty (Specials-tab save clears it), a new class appears, or force=true.
